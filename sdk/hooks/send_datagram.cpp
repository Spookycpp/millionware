...
int __fastcall hooks::send_datagram(c_net_channel *ecx, uintptr_t *edx, void *buffer) {
    ...
    if (buffer || !interfaces::engine_client->is_in_game())
        return send_datagram_hk.call_original<decltype(&send_datagram)>(ecx, edx, buffer);
    ...
    const int ret = send_datagram_hk.call_original<decltype(&send_datagram)>(ecx, edx, buffer);
