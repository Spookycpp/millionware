...
float easing::in_back(float t)
{
	return t * t * (2.70158f * t - 1.70158f);
}

float easing::out_back(float t)
{
	return 1.0f + (--t) * t * (2.70158f * t + 1.70158f);
}

float easing::in_out_back(float t)
{
	return t < 0.5f ? t * t * (7.0f * t - 2.5f) * 2.0f : 1.0f + (--t) * t * 2.0f * (7.0f * t + 2.5f);
}

float easing::in_elastic(float t)
{
	const auto t2 = t * t;
...
float easing::out_elastic(float t)
{
...
float easing::in_out_elastic(float t)
{
...
float easing::in_bounce(float t)
{
	return powf(2.0f, 6.0f * (t - 1.0f)) * fabsf(sinf(t * M_PI * 3.5f));
}

float easing::out_bounce(float t)
{
	return 1.0f - powf(2.0f, -6.0f * t) * fabsf(cosf(t * M_PI * 3.5f));
}

float easing::in_out_bounce(float t)
{
	return t < 0.5f ? 8.0f * powf(2.0f, 8.0f * (t - 1.0f)) * fabsf(sinf(t * M_PI * 7.0f)) : 1.0f - 8.0f * powf(2.0f, -8.0f * t) * fabsf(sinf(t * M_PI * 7.0f));
}
