...
template <typename T> inline T scale_value(T value, T in_min, T in_max, T out_min, T out_max) {
...
inline color_t blend_color(const color_t &start, const color_t &end, float rate) {
    ...
    return color_t((int) std::clamp(start.r + diff_r * rate, 0.0f, 255.0f), (int) std::clamp(start.g + diff_g * rate, 0.0f, 255.0f), (int) std::clamp(start.b + diff_b * rate, 0.0f, 255.0f),
...
class c_element : public std::enable_shared_from_this<c_element> {
  ...
  public:
    ...
    template <typename... T> void add_dependencies(const T &...dependencies) {
