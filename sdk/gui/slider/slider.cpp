...
template <typename T> c_slider<T>::c_slider(std::string_view title, T &value, T value_min, T value_max, std::string_view format) : value_(value) {
    title_ = title;
...
template <typename T> c_slider<T>::c_slider(std::string_view title, T &value, T value_min, T value_max, std::function<std::string(T)> format_fn) : value_(value) {
    title_ = title;
    title_wrap_width_ = 315.0f;
    hover_animation_ = 0.0f;
    value_min_ = value_min;
...
template <typename T> void c_slider<T>::layout(layout_item &overlay, layout_item &parent) {
    ...
    track_head_ =
        track_root_.new_item(LAY_LEFT | LAY_VCENTER).margins(track_head_pos, -var_track_head_size + var_track_bar_height * 0.5f - 1.0f, 0.0f, 0.0f).size(var_track_head_size, var_track_head_size);

    value_item_ = track_root_.new_item(LAY_LEFT | LAY_VCENTER).size(value_size.x, value_size.y).margins(track_head_pos + var_track_head_size * 0.5f - value_size.x * 0.5f + 0.5f, 0.0f, 0.0f, 0.0f);
...
template <typename T> void c_slider<T>::render() {
    ...
    const auto color = color_t(std::clamp((int) (25.0f + (6.0f * hover_animation_)), 0, 255), std::clamp((int) (25.0f + (6.0f * hover_animation_)), 0, 255),
