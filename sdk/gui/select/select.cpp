...
void c_select::layout(layout_item &overlay, layout_item &parent) {
    ...
    if (is_multi_select_) {
        ...
        if (value_.get() == 0) {
            ...
            value_item_ = container_.new_item(LAY_LEFT | LAY_VCENTER).margins(8.0f, 8.0f, 8.0f, 8.0f).size(no_value_selected_size.x, no_value_selected_size.y);
        }
        else {
            auto items_container = container_.new_item(LAY_HFILL | LAY_LEFT, LAY_ROW | LAY_START | LAY_WRAP).margins(8.0f, 8.0f, 4.0f, 4.0f);
            ...
            for (auto i = 0u; i < options_.size(); i++) {
                ...
                auto option_text = option_item.new_item(LAY_LEFT | LAY_VCENTER).margins(4.0f, 2.0f, 0.0f, 2.0f).size(item_text_size.x, item_text_size.y);

                auto option_remove = option_item.new_item(LAY_LEFT | LAY_VCENTER).margins(12.0f, 1.0f, 4.0f, 0.0f).size(remove_size.x, remove_size.y);
    ...
    if (ui::get_blocking() == shared_from_this()) {
        ...
        for (auto i = 0u; i < options_.size(); i++) {
            ...
            auto option_text = option_container.new_item(LAY_LEFT | LAY_VCENTER).margins(8.0f, 6.0f, 8.0f, 6.0f).size(option_size.x, option_size.y);
        }
        ...
        if (!overlay_container_.begin().is_valid()) {
            ...
            no_choices_item_ = overlay_container_.new_item(LAY_LEFT | LAY_VCENTER).margins(8.0f, 8.0f, 8.0f, 8.0f).size(no_choices_size.x, no_choices_size.y);
        }
        else {
...
void c_select::render() {
    ...
    const auto color = color_t(std::clamp((int) (19.0f + (6.0f * hover_animation_)), 0, 255), std::clamp((int) (19.0f + (6.0f * hover_animation_)), 0, 255),
    ...
    render::draw_text({box_pos.x + box_size.x - arrow_size.x - 8.0f, arrow_pos.y}, {255, 255, 255}, ICON_FA_CHEVRON_DOWN, FONT_FA_SOLID_32, 0.0f, 12.0f);

    if (is_multi_select_ && value_.get() == 0) {
        const auto [value_pos, _] = rect_to_xywh(value_item_.get_rect());
        const auto value_color = color_t(std::clamp((int) (150.0f + (20.0f * hover_animation_)), 150, 170), std::clamp((int) (150.0f + (20.0f * hover_animation_)), 150, 170),
        ...
        for (auto i = 0u; i < option_items_.size(); i++) {
            ...
            const auto remove_color = color_t(std::clamp((int) (118.0f + (82.0f * option_item_animation_[i])), 0, 255), std::clamp((int) (118.0f + (82.0f * option_item_animation_[i])), 0, 255),
    ...
    if (!has_been_closed && overlay_container_.is_valid()) {
        ...
        if (!no_choices_item_.is_valid()) {
            ...
            for (auto i = 0u; i < options_.size(); i++) {
                ...
                const auto hover_color = color_t(std::clamp((int) (230.0f + (25.0f * option_hover_animation_[i])), 0, 255), std::clamp((int) (230.0f + (25.0f * option_hover_animation_[i])), 0, 255),
            ...
            render::draw_text(no_choices_pos, {255, 255, 255}, XORSTR("No more choices to choose from"), FONT_CEREBRI_SANS_MEDIUM_18, value_wrap_width_);
        }
