...
void c_tab::layout(int index, layout_item &overlay, layout_item &sidebar, layout_item &content)
{
	...
	tab_item_ = sidebar
		.new_item(LAY_LEFT | LAY_HFILL, LAY_COLUMN | LAY_START)
		.margins(0.0f, index == 0 ? 16.0f : 0.0f, 0.0f, 0.0f);

	tab_inner_item_ = tab_item_
		.new_item(LAY_LEFT, LAY_ROW | LAY_START)
		.margins(0.0f, 8.0f, 0.0f, 8.0f);

	icon_item_ = tab_inner_item_
		.new_item(LAY_LEFT, LAY_ROW)
		.size(22.0f, 16.0f)
		.new_item(LAY_LEFT)
		.size(icon_size.x, icon_size.y);

	name_item_ = tab_inner_item_
	...
	if (gui::get_active_tab() == shared_from_this())
	{
		...
		auto first_row = content
			.new_item(LAY_TOP | LAY_HFILL, LAY_COLUMN | LAY_START)
			.margins(0.0, 0.0, 16.0f, 0.0);
...
void c_tab::render()
{
	...
	const auto color = color_t(
		std::clamp((int) (237.0f + (18.0f * text_animation_)), 0, 255),
