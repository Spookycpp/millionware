...
void c_window::render(bool visualize_layout)
{
	...
	root_ = main_layout_.item(0, LAY_ROW | LAY_START)
		.margins(pos_.x, pos_.y, 0.0f, 0.0f)
		.size(size_.x, size_.y);

	sidebar_ = root_
		.new_item(LAY_VFILL, LAY_COLUMN | LAY_START)
		.size(250.0f, 0.0f);

	content_ = root_
		.new_item(LAY_TOP | LAY_HFILL, LAY_ROW | LAY_WRAP | LAY_START)
		.margins(16.0f, 16.0f, 16.0f, 16.0f);
