...
void scripting::init()
{
	...
	{
		if (strcmp(iter->get_panel_id(), XORSTR("CSGOMainMenu")) == 0)
			break;
...
void scripting::run_script(const std::string &script)
{
	...
	ui_engine->run_script(root_panel, script.data(), XORSTR("panorama/layout/base_mainmenu.xml"), 8, 10, false, false);
}
