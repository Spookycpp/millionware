...
void __fastcall lock_cursor_hook(uintptr_t ecx, uintptr_t edx) {
	const auto _ = std::lock_guard(hooks::lock_cursor.call_mutex);
	const auto ctx = gui::get_window(HASH_FNV_CT("main"));

	interfaces::input_system->enable_input(!ctx.open);
