...
namespace features::aimbot::hitchance
{
	...
	vector3_t get_spread_direction(c_weapon* wpn, const vector3_t& angles, const int seed)
	{
		if (!wpn) {
			return vector3_t();
		}
		...
		const float rand_inaccuracy = rand_a * ctx::client.weapon_inaccuracy;
		const float rand_spread = rand_b * ctx::client.weapon_spread;
	...
	std::vector< hitbox_data_t > get_hitbox_data(C_CSPlayer* target, int hitbox)
	{
		...
		const auto hdr = c_game::instance().get_model_info()->get_studio_model(model);
		...
		if (hitbox == -1)
		{
			for (int i = 0; i < set->num_hitboxes; ++i)
			{
				...
				vector3_t min = math::vector_transform(box->bb_min, bone_matrix[box->bone_index]);
				vector3_t max = math::vector_transform(box->bb_max, bone_matrix[box->bone_index]);
	...
	bool can_hit(C_CSPlayer* target, C_BaseCombatWeapon* wpn, const vector3_t& angles, const int percentage, const int hitbox)
	{
		if (!target || !wpn) {
			return false;
		}

		const auto wpn_info = wpn->get_info();
		...
		const vector3_t eye_pos = ctx::client.local->get_eye_pos();
