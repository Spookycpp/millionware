...
namespace features::lag_compensation
{
	...
	void store_visuals()
	{
		...
		for (int i = 1; i < 65; ++i)
		{
			...
			if (!ent || ent->has_gun_game_immunity())
			{
				data.clear();
				continue;
			}
			...
			if (std::abs(last_sim_time[i] - ent->get_simulation_time()) != 0.0f)
			{
				...
				ent->renderable()->setup_bones(matrices.data(), 128, BONE_USED_BY_ANYTHING, 0.0f);

				for (auto& matrix : matrices)
				{
					const vector3_t origin = ent->origin();
					vector3_t       delta  = math::get_matrix_position(matrix) - origin;
	...
	bool get_render_record(const int idx, matrix3x4_t* out)
	{
		...
		if (!cheat::local_player|| cheat::local_player->life_state() != LIFE_STATE_DEAD) {
			return false;
		}

		auto ent = (c_player*)interfaces::entity_list->get_by_index(idx);
		...
		for (auto it = data.rbegin(); it != data.rend(); ++it)
		{
			...
			if (it->origin.dist(ent->origin()) < 1.0f) {
				return false;
			}
			...
			const vector3_t next = it + 1 == data.rend() ? ent->origin() : (it + 1)->origin;
			const vector3_t lerp = math::lerp(it->origin, next, delta);
			...
			for (int i = 0; i < 128; ++i)
			{
				vector3_t matrix_delta = math::get_matrix_position(it->matrices[i]) - it->origin;
