...
public:
	...
	virtual vector3_t const& player_minimums(bool ducked) const = 0;
	virtual vector3_t const& player_maximums(bool ducked) const = 0;
	virtual vector3_t const& player_view_offset(bool ducked) const = 0;
...
public:
	...
	virtual void get_view_origin(vector3_t& org);
	virtual void set_view_origin(vector3_t& org);
	virtual void get_view_angles(vector3_t& ang);
	virtual void set_view_angles(vector3_t& ang);
	virtual void get_local_view_angles(vector3_t& ang);
	virtual void set_local_view_angles(vector3_t& ang);
	...
	virtual void set_ideal_pitch(int slot, void* player, const vector3_t& origin, const vector3_t& angles, const vector3_t& viewheight);
...
public:
	...
	void run_simulation(int current_command, float curtime, user_cmd_t* cmd, void* local_player);
	void untouch(int slot);
	void store_prediction_results(int slot, int predicted_frame);
	bool should_dump_entity(void* ent);
	void smooth_view_on_moving_platform(void* player, vector3_t& offset);
