...
public:
	...
	VIRTUAL_METHOD(bool, get_player_info, 8, (const int index, player_info_t& buffer), index, &buffer);
	VIRTUAL_METHOD(int, get_index_from_user_id, 9, (const int user_id), user_id);
	...
	VIRTUAL_METHOD(void, execute_cmd, 108, (const char* cmd), cmd);
};
